
<div class="breadLine">            
    <div class="arrow"></div>
    <div class="adminControl active">
        Hi, <?php get_sess_username();?>
    </div>
</div>

<div class="admin">
    <div class="image">
        <img src="/assets/aquarius/img/users/no-image50.jpg" class="img-polaroid"/>                
    </div>
    <ul class="control">                
        <li><span class="icon-comment"></span> <a href="messages.html">Messages</a> <a href="messages.html" class="caption red">12</a></li>
        <li><span class="icon-cog"></span> <a href="/users/setting">Settings</a></li>
        <li><span class="icon-share-alt"></span> <a href="/app/logout">Logout</a></li>
    </ul>
    <div class="info">
        <span>Welcom back! Your last visit: 24.10.2012 in 19:55</span>
    </div>
</div>

<ul class="navigation">            
    <li>
        <a href="/dashboard">
            <span class="isw-grid"></span><span class="text">Dashboard</span>
        </a>
    </li>
    <li>
        <a href="/leads">
            <span class="isw-list"></span><span class="text">Leads</span>
        </a>
    </li>
    <li>
        <a href="/prospects">
            <span class="isw-list"></span><span class="text">Prospect</span>
        </a>
    </li>
    <li>
        <a href="/surveys">
            <span class="isw-list"></span><span class="text">Survey</span>
        </a>
    </li>
    <li>
        <a href="/installs">
            <span class="isw-list"></span><span class="text">Install</span>
        </a>
    </li>
    <li>
        <a href="/clients">
            <span class="isw-list"></span><span class="text">Pelanggan</span>
        </a>
    </li>
</ul>

<div class="dr"><span></span></div>        
